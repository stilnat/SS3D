...
namespace DummyStuff
{
    public class DummyPlace : MonoBehaviour
    {
        ...
        private IEnumerator PlaceReach(DummyHand mainHand, Transform placeTarget, GameObject item)
        {
            ...
            {
                StartCoroutine(DummyTransformHelper.OrientTransformTowardTarget(transform, placeTarget, itemReachDuration, false, true));
            }
            ...
            yield return DummyTransformHelper.LerpTransform(item.transform, placeTarget, itemMoveDuration, true, false, false);
        }
        ...
        private IEnumerator PlaceAndPullBack(DummyHand mainHand, DummyHand secondaryHand, bool withTwoHands)
        {
            ...
            StartCoroutine(CoroutineHelper.ModifyValueOverTime(x => mainHand.pickupIkConstraint.weight = x, 1f, 0f, itemReachDuration));
            ...
            if (withTwoHands)
            {
                StartCoroutine(CoroutineHelper.ModifyValueOverTime(x => secondaryHand.pickupIkConstraint.weight = x, 1f, 0f, itemReachDuration));
                StartCoroutine(CoroutineHelper.ModifyValueOverTime(x => secondaryHand.holdIkConstraint.weight = x, 1f, 0f, itemReachDuration));
            }
